...
2. Sequential Exchangeability: $\underline{Y_t}(g) \perp A_t|H_t, \forall t$. This means all confounding are observed. This would hold if all drivers of treatment decisions that were prognostic for the outcome were observed. This is the key assumption that should be reconsidered in the sensitivity analysis. If this is violated, it means there is still unmeasured confounding beyond our covariates in the model. If there is indeed a very important missing covariate, it will lead to the underfitting of the neural network so that the simulation and estimation will be influenced. But it is not so clear how much the influence is because estimating underfitting of a neural network depends on the context.

3. Positivity: $P(A_t = g_t(H_t)) > 0, \forall \lbrace H_t:P(H_t) > 0\rbrace$. This means the counterfactual treatment strategy of interest has some non-zero probability of actually being followed. Positivity is not strictly necessary. If this assumption is violated, some counterfacutal treatment will not be within our feasible set, which may make the algorithm fail to make meaningful prediction. The violation can be detected when the algorithm encounters a new treatment which has not been considered. If that is the case, we need to go back checking the dataset to make sure that this treatment has related observations. This assumption is regular assumption that needs not to be considered in the sensitivity analysis. 
